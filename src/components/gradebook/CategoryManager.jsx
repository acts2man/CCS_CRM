...
export default function CategoryManager({ classSection, categories, onClose, onSaved }) {
  ...
  const save = async () => {
    ...
    setSaving(true);
    // Delete existing categories for this class
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-lg">
        <DialogHeader><DialogTitle>Grade Categories — {classSection.name}</DialogTitle></DialogHeader>
        <div className="space-y-3 pt-2">
          <p className="text-sm text-gray-500">Define weighted grade categories. Total must equal exactly 100%.</p>

          {items.map((item, idx) => (
            <div key={idx} className="flex gap-2 items-end">
              <div className="flex-1">
                {idx === 0 && <Label className="text-xs">Category Name</Label>}
          ...
          <Button variant="outline" size="sm" onClick={addRow}><Plus className="h-4 w-4 mr-1" /> Add Category</Button>
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button className="bg-slate-900 hover:bg-slate-800" onClick={save} disabled={totalWeight !== 100 || saving}>
              {saving && <Loader2 className="h-4 w-4 animate-spin mr-2" />} Save Categories
            </Button>
          </div>
        </div>
      </DialogContent>
